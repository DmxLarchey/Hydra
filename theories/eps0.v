...
From Hydra Require Import utils ordinal ordered lex2 lex_list list_order wlist E0.
...
Section eps0.
  ...
  Proof. now exists (E0_zero o). Defined.

  Definition eps0_one : ε₀.
  Proof. now exists (E0_one o). Defined.
  ...
  Hint Resolve eps0_le_add_left eps0_lt_add_right : core.

  Section eps0_add_extra.

  Fact eps0_le_add e e' f f' : e ≤ε₀ e' → f ≤ε₀ f' → e +₀ f ≤ε₀ e' +₀ f'.
  ...
  Fact eps0_le_add_incr_right e f : e ≤ε₀ e +₀ f.
  Proof. rewrite <- (eps0_add_zero_right e) at 1; auto. Qed.

  Hint Resolve eps0_le_add_incr_left eps0_le_add_incr_right : core.

  Fact eps0_lt_add_cancel e u v : e +₀ u <ε₀ e +₀ v → u <ε₀ v.
  Proof. 
    intros H.
    destruct (eps0_lt_sdec u v) as [ u v ? | u | u v G ]; auto.
    + now apply eps0_lt_irrefl in H.
    + apply eps0_lt_add_right with (e := e) in G.
  ...
  Proof.
    ...
    + apply eps0_lt_add_incr with (e := e) in Hf.
      rewrite H in Hf.
  ...
  Proof.
    ...
    + right.
      ...
      apply eps0_lt_add_cancel in H; eauto.
  Qed.
  ...
  Proof.
    ...
    destruct (eps0_lt_sdec u v) as [ u v G | u | u v G ]; auto;
      apply eps0_lt_add_right with (e := e) in G; rewrite H in G;
      edestruct eps0_lt_irrefl; eauto.
  Qed.

  Fact eps0_add_le_cancel e u v : e +₀ u ≤ε₀ e +₀ v → u ≤ε₀ v.
  Proof. rewrite !eps0_le_iff_lt; now intros [ ?%eps0_lt_add_cancel | ?%eps0_add_cancel ]; [ left | right ]. Qed.

  End eps0_add_extra.

  Hint Resolve eps0_le_lt_trans
               eps0_lt_succ
               eps0_le_not_succ : core.

  Hint Resolve eps0_lt_add_incr
               eps0_le_add_incr_left
               eps0_le_add_incr_right : core.

  Hint Resolve eps0_le_add_left
               eps0_lt_add_right
               eps0_le_add : core.

  Section eps0_omega.

  Implicit Types (n m : o).

  Hint Resolve cnf_sg : core.

  (* ω^e.p where p is positive *)
  Definition eps0_exp : ε₀ → o → ε₀.
  Proof.
    intros (e & He) p.
  ...
  Hint Resolve eps0_le_exp ord_le_refl ord_lt_le_weak eps0_le_refl: core.
  ...
  Proof. unfold eps0_omega; rewrite eps0_add_exp, ord_add_zero_right; auto. Qed.
  ...
  Proof.
    ...
    + intros [ | (-> & []%ord_le_lt_iff) ]; subst; auto.
  Qed.
  ...
  Fact eps0_add_exp_hnf_eq e i₁ i₂ f₂ :
      f₂ <ε₀ ω^e
    → ω^⟨e,i₁⟩ +₀ (ω^⟨e,i₂⟩ +₀ f₂) = ω^⟨e,i₁ +ₒ 1ₒ +ₒ i₂⟩ +₀ f₂.
  Proof.
    intro.
    rewrite <- (eps0_add_zero_right ω^⟨e,i₁⟩).
  ...
  Fact eps0_le_add_exp_omega e n m : n <ₒ m → ω^⟨e,n⟩ +₀ ω^e ≤ε₀ ω^⟨e,m⟩.
  Proof.
    intros H%ord_lt__succ_le_iff; unfold eps0_omega.
    rewrite eps0_add_exp, ord_add_zero_right; auto.
  Qed.
  ...
  Proof.
    ...
    + rewrite eps0_add_zero_right.
      apply eps0_lt_le_trans with (2 := eps0_le_add_exp_omega _ _ _ H); auto.
    + rewrite eps0_add_assoc.
      apply eps0_lt_le_trans with (2 := eps0_le_add_exp_omega _ _ _ H); auto.
      apply eps0_lt_add_right, eps0_add_below_omega; auto.
...
(* Hence a successor is not a limit ordinal 
  ...
  Proof. rewrite <- eps0_add_one_right; auto. Qed.
  ...
  Fact eps0_is_succ_exp_zero n : eps0_is_succ ω^⟨0₀,n⟩ ↔ ord_is_succ (1ₒ +ₒ n).
  Proof.
    split.
    + intros (l & i & H1 & H2)%eps0_is_succ_iff%E0_is_succ_iff.
      2: apply proj2_sig.
      simpl in H1; apply E0_eq_inv in H1.
      rewrite <- (app_nil_l [_]) in H1 at 1.
      apply app_inj_tail in H1 as (<- & [=]); subst; auto.
    + intros [ -> | (j & ->) ]%ord_is_succ_1add_inv.
      * exists 0₀; rewrite eps0_add_zero_left; apply eps0_omega_zero.
      * exists ω^⟨0₀,j⟩; now rewrite eps0_add_one_right, eps0_succ_exp.
  Qed.
  ...
  Fact eps0_is_limit_iff_E0 e : eps0_is_limit e ↔ E0_is_limit (π₁ e).
  Proof.
  ...
  Fact eps0_is_limit_exp_not_zero e n : e ≠ 0₀ → eps0_is_limit ω^⟨e,n⟩.
  Proof.
    intros H.
    apply eps0_is_limit_iff_E0.
    destruct e; simpl.
  ...
  Proof.
    ...
    + destruct e as (e & He); rewrite eps0_is_limit_iff_E0; simpl.
      rewrite E0_is_limit_iff; auto; simpl.
      intros (l & b & i & -> & H).
      assert (Hl : cnf (E0_cons l)) by now apply cnf_app_left_inv in He.
      assert (Hb : cnf b) by now apply cnf_app_right_inv, cnf_cons_inv_left in He. 
      exists (exist _ _ Hl), (exist _ _ Hb), i; rewrite !eps0_eq_iff; simpl; split; auto.
      now apply E0_cnf_add_snoc.
    + intros (a & b & i & -> & C).
      apply eps0_add_is_limit_iff; right.
      apply eps0_is_limit_iff_E0. 
      clear a.
      destruct b as (b & Hb); simpl.
      rewrite !eps0_eq_iff in C; simpl in C.
      apply E0_is_limit_iff; auto.
      exists [], b, i; auto.
  Qed.

  Fact eps0_is_limit_exp_zero n : n ≠ 0ₒ → ¬ ord_is_succ (1ₒ +ₒ n) → eps0_is_limit ω^⟨0₀,n⟩.
  Proof.
  ...
  Fact eps0_hnf_is_limit e n f :
      eps0_is_limit (ω^⟨e,n⟩ +₀ f)
    ↔ eps0_is_limit f ∨ f = 0₀ ∧ (e ≠ 0₀ ∨ e = 0₀ ∧ ord_is_limit n).
  ...
  Proof.
    destruct e as [ | n | e i f ? ? _ _ ] using eps0_hnf_pos_rect.
    + now left.
    + right.
      destruct (ord_zero_or_1add n) as [ -> | (k & ->) ].
      * exists 0₀; now rewrite eps0_add_zero_right, <- eps0_omega_zero.
      * exists ω^⟨0₀,k⟩.
        rewrite <- eps0_omega_zero.
        unfold eps0_omega.
        now rewrite eps0_add_exp, ord_add_zero_left.
    + right.
      exists (ω^⟨e,i⟩ +₀ f).
...
#[global] Infix "<ε₀" := (@eps0_lt _).
#[global] Infix "≤ε₀" := (@eps0_le _).

#[global] Notation "0₀" := (@eps0_zero _).
#[global] Notation "1₀" := (@eps0_one _).
#[global] Notation S₀ := (@eps0_succ _).
#[global] Infix "+₀" := (@eps0_add _).

#[global] Notation "ω^⟨ e , i ⟩" := (@eps0_exp _ e i).
#[global] Notation "ω^ e" := (@eps0_omega _ e).
